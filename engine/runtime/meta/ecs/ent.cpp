#include "./ent.hpp"

#include <core/serialization/associative_archive.h>
#include <core/serialization/binary_archive.h>
#include <core/serialization/types/vector.hpp>
#include <core/system/subsystem.h>

namespace runtime
{
std::map<std::uint64_t, EntityType>& get_serialization_map()
{
 /// Keep count of serialized entities
 static std::map<std::uint64_t, EntityType> serialization_map;
 return serialization_map;
}

SAVE(EntityType)
{
 // TODO check for validity
  // cereal::process(obj);
  ar(obj);
 // auto id = obj.id().id();
 // try_save(ar, cereal::make_nvp("entity_id", id));

 // if(obj.valid())
 // {
 //   auto& serialization_map = get_serialization_map();
 //   auto it = serialization_map.find(id);
 //   if(it == serialization_map.end())
 //   {
 //     serialization_map[id] = obj;

 //     try_save(ar, cereal::make_nvp("name", obj.get_name()));
 //     try_save(ar, cereal::make_nvp("components", obj.all_components()));
 //   }
 // }
}
SAVE_INSTANTIATE(EntityType, cereal::oarchive_associative_t);
SAVE_INSTANTIATE(EntityType, cereal::oarchive_binary_t);

LOAD(EntityType)
{
    ar(obj);
 // std::uint64_t id = 0;
 // std::string name;
 // std::vector<chandle<component>> components;

 // try_load(ar, cereal::make_nvp("entity_id", id));

 // if(id != EntityType ::INVALID.id())
 // {
 //   auto& serialization_map = get_serialization_map();
 //   auto it = serialization_map.find(id);
 //   if(it != serialization_map.end())
 //   {
 //     obj = it->second;
 //   }
 //   else
 //   {
 //     auto& ecs = core::get_subsystem<SpatialSystem>();
 //     obj = ecs.create();
 //     serialization_map[id] = obj;

 //     try_load(ar, cereal::make_nvp("name", name));
 //     try_load(ar, cereal::make_nvp("components", components));

 //     obj.set_name(name);
 //     for(const auto& component : components)
 //     {
 //       auto component_shared = component.lock();
 //       if(component_shared)
 //       {
 //         obj.assign(component_shared);
 //         component_shared->touch();
 //       }
 //     }
 //   }
 // }
}

LOAD_INSTANTIATE(EntityType, cereal::iarchive_associative_t);
LOAD_INSTANTIATE(EntityType, cereal::iarchive_binary_t);
}
